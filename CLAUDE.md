...
| 일정 | 일차, 순서, 장소, 일정명, 예정시각, 대상(전체/선발/후발), 셔틀여부(출발/귀가/빈칸), 항공구간(가는편/오는편/빈칸), 항공사 필터, 공지 | 1, 1, 국내선 3번 게이트, 김포공항 탑승 게이트 집결, 08:30, 전체, , 가는편, 티웨이, "수하물 찾으면 조 단위로 가이드 만남" |
...
- 이메일 중복 시 검증 오류 (동명이인 대응)
- 로그인 가능 역할: 조장, 관리자, 관리자(조장). 조원은 로그인해도 `/no-access`로 라우팅
...
| airline_filter | text | nullable | 비행편 필터 키워드 (예: `'티웨이'`, `'제주항공'`, `'에어서울'`). `users.airline`/`return_airline` 부분 매칭(`includes`)으로 조원에 해당 편 탑승자 0명이면 조장 화면에서 일정 자동 숨김. null이면 필터 적용 안 함 |
| notice | text | nullable | 일정별 공통 안내. 모든 조장에게 동일하게 조장 브리핑 바텀시트에 표시. 예: `"수하물 찾으면 조 단위로 가이드 만남"`, `"버스에서 방장이 카드키·조식쿠폰 수령"`. null이면 블록 렌더 안 함 |
| created_at | timestamptz | default now() | 생성 시각 |
...
- **진행중 일정:** 노란 테두리(`#FEE500`) 강조 + '진행중' pill + 완료/전체 카운트 → **카드 탭 → 체크인 화면 진입** (단일 액션)
...
- **전체 현황:** 조장 뷰에는 없음. 조장은 자기 조에만 집중(역할 분리). 다른 조 현황은 관리자 화면에서만 조회
...
| 진행바 | `#0ACF83` | 프로그레스 바 fill (1.4.11 의도적 완화 — 텍스트+ARIA 보완) |
| 오프라인 | `#F1EFE8` | 하단 오프라인 배너 |
| 진행중 배지 | `#FEF9C3` | 진행중 배지 배경 (yellow-100, dot: yellow-500, text: yellow-900) |
